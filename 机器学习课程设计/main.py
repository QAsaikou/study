...
startdate = datetime.datetime.strptime('2024-06-18', '%Y-%m-%d')
...
print("数据预处理完成.")
testA=data[data['fraud'].isnull()].drop(['policy_id',
                                         'incident_date',
                                         'fraud'],
                                            axis=1)
trainA=data[data['fraud'].notnull()]
data_x=trainA.drop(['policy_id',
                    'incident_date',
                    'fraud'],
                    axis=1)
...
sk = StratifiedKFold(n_splits=n_folds,
                     shuffle=True,
                     random_state=2019)
...
for train, test in sk.split(data_x, data_y):
    ...
    print('CatBoost验证的Acu:{}'.format(roc_auc_score(y_test, yy_pred_valid)))
...
print('10折平均Acu:{}'.format(mean_score))
